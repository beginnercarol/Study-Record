window.onload = function(){
    //轮播图片
  (function(){
    var slogans=["数学建模专业团队","985、211院校本科硕士博士","各类数模比赛预定"];
    ...
    var imgSmBarcode = document.querySelector('img.smBarcode');
    var imgXlBarcode = document.querySelector('span.bigBarcode');
    imgSmBarcode.onclick = function(){
        console.log(imgXlBarcode.className);
        let xlClass = imgXlBarcode.className;
        let newClass = 'bigBarcodeActive';
        xlClass = xlClass.split(" ");
        let index  = xlClass.indexOf(newClass);
        if(index>=0){
            xlClass = (xlClass.slice(0,index).concat(xlClass.slice(index+1))).join(" ");
        }else{
            xlClass = xlClass.concat(newClass).join(" ");
            console.log(xlClass);
        }
        imgXlBarcode.className = xlClass;
        console.log(imgXlBarcode.className);
    }
  })();
